...
foreach($row in $csv){
    ...
    #remove DLR Bridge
    Start-Job -Name  RemoveLSBridge  -ScriptBlock $RemoveLSBridge 
     
    # Connect LS to DLR
    Start-Job -name ConnectLStoDLR -ScriptBlock $ConnectLStoDLR -ArgumentList $dlrName,$primaryAddress,$prefixLength,$DefaultNSXConnection
